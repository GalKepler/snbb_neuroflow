...
The Pipeline Registry provides a centralized definition of all available pipelines, their dependencies, and execution requirements. This enables proper dependency resolution and workflow orchestration.

## File: `neuroflow/orchestrator/registry.py`
...
for pipeline in registry.get_execution_order():
    print(f"{pipeline.name}: {pipeline.display_name}")

# Get dependencies for a pipeline
deps = registry.get_dependencies("qsirecon")
print(f"QSIRecon depends on: {[d.name for d in deps]}")

# Get all session-level pipelines
session_pipelines = registry.get_by_level(PipelineLevel.SESSION)
